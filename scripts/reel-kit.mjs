// WyberAi — 9:16 Reel/Shorts starter kit in brand design language.
...
// Output: OneDrive\Desktop\Wyber Ai\reels-kit\  (Desktop is OneDrive-redirected)
...
const OUT = path.join(os.homedir(), 'OneDrive', 'Desktop', 'Wyber Ai', 'reels-kit')
...
const frames = [
  ...
  // 2 — hook cover (feed thumbnail + first frame). Swap the hook per reel.
  { name: 'reel-02-hook-cover.png', html: page(glow, stack(`
    <div class="mono" style="font-size:30px;color:#0EA5E9;letter-spacing:.1em">NO CODE. NO KIDDING.</div>
    <div style="font-weight:900;font-size:120px;letter-spacing:-0.05em;color:#f4f4f5;line-height:1.02">One sentence in.<br><span class="sky">A real app out.</span></div>
    <div style="font-size:40px;color:#a1a1aa;line-height:1.4;max-width:820px">Watch me build & deploy a full app<br>by just typing what I want 👇</div>`, 52)) },

  // 3 — value text card (talking-point style)
  { name: 'reel-03-card-freshcode.png', html: page(glow, stack(`
    ${lockup(56,38)}
    <div style="font-weight:900;font-size:110px;letter-spacing:-0.05em;color:#f4f4f5;line-height:1.04">No templates.<br><span class="sky">Fresh code<br>every time.</span></div>
    <div style="font-size:40px;color:#a1a1aa;line-height:1.45;max-width:820px">Every project is written from scratch — real React, not a drag-and-drop clone.</div>`, 56)) },

  // 4 — value text card (differentiator)
  { name: 'reel-04-card-selfheal.png', html: page(glow, stack(`
    ${lockup(56,38)}
    <div style="font-weight:900;font-size:104px;letter-spacing:-0.045em;color:#f4f4f5;line-height:1.05">Other builders<br>generate & <span style="color:#71717a">hope.</span><br><span class="sky">Wyber engineers it.</span></div>
    <div style="font-size:40px;color:#a1a1aa;line-height:1.45;max-width:840px">Self-healing builds. Live database security scans. Fresh code every time.</div>`, 56)) },
...
fs.writeFileSync(path.join(OUT, 'README.txt'), `WyberAi — 9:16 Reel Kit (1080x1920, safe-zoned for IG/TikTok/Shorts/LinkedIn)
===========================================================================
Drop these around your screen-recording in CapCut or InShot.

reel-01-intro.png          Brand open (1s at the very start)
reel-02-hook-cover.png     Hook + cover thumbnail — SWAP the hook text per reel
reel-03-card-freshcode.png Value card — "fresh code every time"
reel-04-card-selfheal.png  Value card — "we engineer it" differentiator
reel-05-outro-follow.png   End card — FOLLOW cta (not a sales cta)

Typical reel = intro (1s) -> hook (2s) -> your screen-recording of a build ->
1-2 value cards over voiceover -> outro follow card (2s). 15-40s total.
`)
console.log('\\nReel kit in:', OUT)
