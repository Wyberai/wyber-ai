// Verifies prebuilt_apps rows using the SAME esbuild settings as the real
// production preview engine (src/lib/wyber-preview/engine.ts): jsx:'automatic'
// (not 'transform'), same external-deps list, same virtual-fs resolution
// logic. Written because a Node-native-esbuild bundle-check using jsx:'transform'
// passed 0 failures for a batch, yet the live browser preview (esbuild-wasm,
// jsx:'automatic') surfaced a real build error — need to know whether that
// mismatch is real or the failing template is unrelated to this batch.
...
const catFilter = process.argv[2] ? process.argv[2].split(',') : ['Ecommerce', 'WebApp-Ecommerce']
const { data, error } = await admin.from('prebuilt_apps').select('id,name,category,files').in('category', catFilter).eq('valid', true)
