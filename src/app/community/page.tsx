...
export const metadata: Metadata = { title: 'Community — Wyber AI', description: 'Join the Wyber AI community. Connect with founders, designers, and developers building with AI.' };
const CHANNELS = [
  { icon: '💬', name: 'Discord', desc: 'Real-time help, share what you\'re building, and connect with other builders.', cta: 'Join Discord', href: 'https://discord.gg/A5KsFv2P', color: '#5865F2' },
  { icon: '𝕏', name: 'Twitter / X', desc: 'Product updates, tips, and what the community is shipping.', cta: 'Follow @wyberai', href: 'https://twitter.com/wyberai', color: '#000000' },
  { icon: '⌥', name: 'GitHub', desc: 'Open source components, templates, and integrations.', cta: 'View GitHub', href: 'https://github.com/Wyberai', color: '#24292E' },
  { icon: '📋', name: 'Reddit', desc: 'r/wyberai — share your builds, ask questions, get feedback.', cta: 'Join subreddit', href: 'https://reddit.com/r/wyberai', color: '#FF4500' },
];
const PERKS = [
  { emoji: '🎟', title: 'Founding member badge', desc: 'Join early and get a permanent founding member badge on your profile.' },
  { emoji: '🚀', title: 'First access to features', desc: 'Community members get early access to every new feature before public launch.' },
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--bg)', fontFamily:'var(--font-sans)' }}>
      ...
      <div className="wy-section" style={{ paddingBottom:0, textAlign:'center' }}>
        ...
        <p style={{ fontSize:17, color:'var(--text2)', maxWidth:480, lineHeight:1.75, margin:'0 auto 36px' }}>Wyber AI is early. The community is small, tight, and growing fast. Join now and help shape what it becomes.</p>
        <a href="https://discord.gg/A5KsFv2P" target="_blank" rel="noreferrer" className="wy-btn-primary" style={{ display:'inline-flex', marginBottom:56 }}>Join Discord — it's free →</a>
      </div>
      <div className="wy-section" style={{ paddingTop:32 }}>
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(240px,1fr))', gap:12, marginBottom:52 }}>
          {CHANNELS.map(c=>(
            <div key={c.name} className="wy-card" style={{ padding:'24px' }}>
              ...
              <a href={c.href} target="_blank" rel="noreferrer" style={{ fontSize:12, fontWeight:700, color:'var(--sky)' }}>{c.cta} ↗</a>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding:'48px', borderRadius:20, background:'linear-gradient(135deg, var(--sky3), var(--bg2))', border:'1px solid var(--border)', textAlign:'center' }}>
          <h2 style={{ fontFamily:'var(--font-serif)', fontSize:'clamp(24px,3.5vw,40px)', fontWeight:400, color:'var(--text)', letterSpacing:'-0.025em', marginBottom:12 }}>Ready to build with others?</h2>
