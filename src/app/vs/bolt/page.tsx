...
export const metadata: Metadata = {
  title: 'Wyber AI vs Bolt.new — Honest Comparison 2026',
  description: 'Wyber AI vs Bolt.new: full feature comparison, pricing, and credits. 50% more credits at 75% of the price. Updated May 2026.',
  openGraph: {
    title: 'Wyber AI vs Bolt.new — Which is better?',
    description: 'Honest comparison: features, pricing, credits. Updated May 2026.',
    images: [{ url: 'https://wyberai.com/api/og?title=Wyber%20AI%20vs%20Bolt.new&sub=Honest%20comparison%20%E2%80%94%20Updated%202026', width: 1200, height: 630 }],
  },
}
...
const ROWS = [
  ...
  ['Deploy to Netlify',      'Coming soon',   '✓',              'bolt'],
  ['GitHub sync',            '✓',             '✓',              'tie'],
...
export default function VSBoltPage() {
  ...
  const boltWins = ROWS.filter(r => r[3] === 'bolt').length
    ...
    <div style={{ minHeight: '100vh', background: '#09090b', color: '#fafafa', fontFamily: "'Space Grotesk', sans-serif" }}>
      ...
      <div style={{ maxWidth: 820, margin: '0 auto', padding: 'clamp(40px,6vw,80px) clamp(16px,4vw,48px)' }}>
        <div style={{ textAlign: 'center', marginBottom: 52 }}>
          ...
          <h1 style={{ fontFamily: "'Sora', sans-serif", fontSize: 'clamp(28px,5vw,52px)', fontWeight: 800, letterSpacing: '-0.04em', marginBottom: 14, lineHeight: 1.1 }}>Wyber AI vs Bolt.new</h1>
          <p style={{ fontSize: 16, color: '#71717a', maxWidth: 500, margin: '0 auto' }}>Bolt.new is a developer-focused AI builder with WebContainers. Wyber AI is the affordable, non-technical-first alternative with more credits and a lower price.</p>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: 12, marginBottom: 40, alignItems: 'center' }}>
          ...
          <div style={{ padding: 20, borderRadius: 14, background: '#111113', border: '1px solid rgba(255,255,255,0.07)', textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: "'Sora', sans-serif", fontSize: 36, fontWeight: 800, color: '#a1a1aa', margin: '0 0 2px' }}>{boltWins}</div>
            <div style={{ fontSize: 12, color: '#71717a' }}>categories won</div>
          </div>
        </div>
        <div style={{ borderRadius: 14, border: '1px solid rgba(255,255,255,0.07)', overflow: 'hidden', marginBottom: 40 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', background: '#111113', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
            {['Feature', 'Wyber AI', 'Bolt.new'].map((h, i) => <div key={h} style={{ padding: '12px 16px', fontSize: 11, fontWeight: 700, color: i === 1 ? '#0EA5E9' : '#52525b', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{h}</div>)}
          </div>
          {ROWS.map((row, i) => (
            <div key={row[0]} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', borderBottom: i < ROWS.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none', background: row[3] === 'wyber' ? 'rgba(14,165,233,0.03)' : 'transparent' }}>
              ...
              <div style={{ padding: '12px 16px', fontSize: 13, color: row[3] === 'bolt' ? '#fafafa' : '#52525b' }}>{row[2]}</div>
            </div>
          ))}
        </div>
