...
const CONNECTORS: { id: string; name: string; desc: string; icon: string; color: string; category: string; prompt: string; logoSlug?: string; secretKeys?: SecretKey[] }[] = [
  ...
  { id: 'whatsapp-business', name: 'WhatsApp Business', desc: 'Order updates & support via WhatsApp Cloud API', icon: '💚', color: '#25D366', category: 'Email & Messaging', prompt: 'Add WhatsApp Business Cloud API integration for sending order confirmations, OTPs, and customer support messages, and receiving replies via the webhook.', secretKeys: [{ name: 'WHATSAPP_PHONE_NUMBER_ID', placeholder: 'phone number id' }, { name: 'WHATSAPP_ACCESS_TOKEN', placeholder: 'access token' }, { name: 'WHATSAPP_VERIFY_TOKEN', placeholder: 'webhook verify token' }] },
