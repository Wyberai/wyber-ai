...
export function EnvironmentsPanel({ projectId, publishedUrl }: Props) {
  ...
  const promote = async () => {
    ...
    if (data.environment) { setEnvs(prev => [...prev.filter(e => e.name !== 'live'), data.environment]); setMsg('✓ Promoted to Live successfully'); }
    else setMsg('Failed: ' + (data.error || 'Unknown error'));
    setPromoting(false);
  };

  const restore = async () => {
    if (!confirm('Restore Live snapshot to Test? This will overwrite your current work.')) return;
    setRestoring(true);
    ...
    setMsg('✓ Restored from Live snapshot');
    setRestoring(false);
  };

  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '12px 0' }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', letterSpacing: '0.08em' }}>Test / Live Environments</div>
      <div style={{ fontSize: 11, color: 'var(--text3)', lineHeight: 1.6 }}>Build safely in Test. When ready, promote to Live. Never break production.</div>

      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
        <div style={{ padding: '12px', borderRadius: 10, border: '1px solid var(--border)', background: 'var(--bg2)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
            <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#F59E0B' }} />
            <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text)' }}>Test</span>
          </div>
          <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 8 }}>Your current working version. Safe to experiment.</div>
          <div style={{ fontSize: 10, padding: '2px 7px', borderRadius: 20, background: 'rgba(245,158,11,0.1)', color: '#F59E0B', fontWeight: 600, display: 'inline-block' }}>Active</div>
        </div>
        ...
        <div style={{ padding: '12px', borderRadius: 10, border: `1px solid ${liveEnv ? 'rgba(52,211,153,0.3)' : 'var(--border)'}`, background: liveEnv ? 'rgba(52,211,153,0.04)' : 'var(--bg2)' }}>
          ...
          {liveEnv ? (
            <>
              <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 6 }}>Promoted {new Date(liveEnv.promoted_at).toLocaleDateString()}</div>
            ...
            <div style={{ fontSize: 11, color: 'var(--text3)' }}>No live version yet. Promote from Test when ready.</div>
          )}
        </div>
      </div>
      ...
      {liveEnv && (
        <button onClick={restore} disabled={restoring} style={{ padding: '8px', borderRadius: 9, border: '1px solid var(--border)', background: 'transparent', color: 'var(--text2)', fontSize: 12, cursor: 'pointer', fontFamily: 'inherit' }}>
          {restoring ? 'Restoring...' : '↓ Restore from Live snapshot'}
        </button>
      )}
