...
export function ClonePanel({ onClose }: { onClose?: () => void }) {
  ...
  const clone = async () => {
    ...
    addMessage({ id: msgId, role: 'user', content: `${t('cloneChatUserPrefix')} ${cleanUrl}`, timestamp: 0, status: 'done' })
    const aId = Math.random().toString(36).slice(2)
    addMessage({ id: aId, role: 'assistant', content: t('cloneScrapingMessage').replace('{url}', cleanUrl), timestamp: 0, status: 'streaming' })
    ...
    try {
      ...
      addMessage({ id: aId, role: 'assistant', content: t('cloneClonedMessage').replace('{url}', cleanUrl).replace('{count}', String(newFiles.length)), timestamp: 0, status: 'done', filesChanged: newFiles.map(f => f.path) })
    } catch (err: any) {
      setError(err.message || t('cloneErrorFallback'))
      addMessage({ id: aId, role: 'assistant', content: `${t('cloneFailedPrefix')} ${err.message}`, timestamp: 0, status: 'error' })
    }
  ...
  return (
    <div style={{ padding: 16, background: 'var(--bg-base)', height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>{t('cloneTitle')}</div>
      ...
      <button onClick={clone} disabled={!url.trim() || loading}
        ...
        {loading ? t('cloneCloning') : t('cloneButton')}
      </button>
