...
// The in-app (RN-web) bundler stays accessible via the toggle.
...
export function MobilePreviewPanel() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem('wyber:mobile-preview-mode:v6')
  ...
  const chooseMode = (m: PreviewMode) => {
    setMode(m)
    try { localStorage.setItem('wyber:mobile-preview-mode:v6', m) } catch { /* private mode */ }
  }
