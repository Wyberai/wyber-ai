// Self-contained Mario-style infinite runner shown in the preview panel
// while the first build is generating. No external deps — pure Canvas 2D.
// Displayed via an iframe with srcDoc so it tears down cleanly when html lands.
...
html,body{width:100%;height:100%;overflow:hidden;background:#09090b;font-family:system-ui,sans-serif}
#hdr{
  position:fixed;top:0;left:0;right:0;height:40px;
  display:flex;align-items:center;justify-content:space-between;
  padding:0 14px;background:#09090b;
  border-bottom:1px solid rgba(139,92,246,0.25);z-index:10
}
#hdr-l{display:flex;align-items:center;gap:8px}
.dot{width:8px;height:8px;border-radius:50%;background:#8b5cf6;animation:pulse 1.4s infinite}
#hdr-t{font:600 12px system-ui;color:#a78bfa}
#hdr-s{font:600 12px monospace;color:#fbbf24}
#cvs{position:fixed;top:40px;left:0;right:0;bottom:28px;display:block}
#ftr{
...
(function(){
  ...
  function reset() {
    ...
    plr.y = groundTop() - plr.h;
    plr.vy = 0; plr.onGround = true; plr.jumps = 0;
  ...
  function spawnPlatform() {
    ...
    if (Math.random() > 0.35) {
      coins.push({ x: pX + pW / 2, y: pY - 22, r: 9, alive: true });
    }
  }
  ...
  function update() {
    ...
    for (var i = 0; i < bugs.length; i++) {
      ...
      if (plr.x + plr.w - 4 > b.x && plr.x + 4 < b.x + b.w &&
        ...
        dead = true;
        addParticles(plr.x + plr.w / 2, plr.y + plr.h / 2, '#8b5cf6');
        return;
      }
    }
  ...
  function drawPlayer() {
    ...
    ctx.fillRect(x + 2, y + 9, plr.w - 4, plr.h - 9);
    // Head
    ctx.fillStyle = '#fde68a';
    ctx.beginPath();
    ctx.arc(x + plr.w / 2, y + 7, 7, 0, Math.PI * 2);
    ctx.fill();
    // Hard hat
    ctx.fillStyle = '#f59e0b';
    ctx.fillRect(x + 2, y + 1, plr.w - 4, 5);
    ctx.fillRect(x - 1, y + 4, plr.w + 2, 3);
    // Eye
    ctx.fillStyle = '#1c1917';
    ctx.fillRect(x + plr.w / 2 + 1, y + 5, 2, 2);
    // Arms
    ctx.fillStyle = '#6d28d9';
    ctx.fillRect(x - 3, y + 11, 4, 8);
    ctx.fillRect(x + plr.w - 1, y + 11, 4, 8);
    // Legs
    if (plr.onGround) {
      ctx.fillRect(x + 3, y + plr.h - 8, 6, 8);
      ctx.fillRect(x + plr.w - 9, y + plr.h - 8, 6, 8);
      if (leg === 0) ctx.fillRect(x + 3, y + plr.h - 10, 6, 8);
      else ctx.fillRect(x + plr.w - 9, y + plr.h - 10, 6, 8);
    } else {
      ctx.fillRect(x + 3, y + plr.h - 6, 6, 6);
      ctx.fillRect(x + plr.w - 9, y + plr.h - 6, 6, 6);
    }
  }
  ...
  function draw() {
    ...
    for (var i = 0; i < bugs.length; i++) {
      ...
      ctx.fillRect(b.x + 4, b.y - 5, 2, 5);
      ctx.fillRect(b.x + b.w - 6, b.y - 5, 2, 5);
      // Eyes
      ctx.fillStyle = '#fff';
      ctx.fillRect(b.x + 2, b.y + 3, 5, 4);
      ctx.fillRect(b.x + b.w - 7, b.y + 3, 5, 4);
      ctx.fillStyle = '#991b1b';
      ctx.fillRect(b.x + 4, b.y + 4, 2, 2);
      ctx.fillRect(b.x + b.w - 6, b.y + 4, 2, 2);
